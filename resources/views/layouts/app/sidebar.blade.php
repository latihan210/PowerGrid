...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:sidebar sticky collapsible class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:sidebar.nav>
            ...
            <flux:sidebar.group icon="user-group" :heading="__('Data Users')" class="grid" expandable
                ...
                <flux:sidebar.item icon="key" :href="route('roles.index')" :current="request()->routeIs('roles.index')"
                    wire:navigate>
                    {{ __('Role List') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="shield-check" :href="route('permissions.index')"
    ...
    <script>
        document.addEventListener('livewire:navigated', () => {
                Flux.toast({ variant: '{{ session('flux.toast.variant') }}', text: '{{ session('flux.toast.text') }}' });
            }, { once: true });
    </script>
    @endif
    @endpersist
